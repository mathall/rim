...
impl Window {
    ...
    fn caret(&self) -> &Caret {
        self.caret_for(self.buf_id)
            .expect("Window lacked state for its buffer id.")
    }
...
impl Rim {
    ...
    fn move_focus(&mut self, direction: frame::Direction) {
        self.frame
            .get_adjacent_window(&self.frame_ctx, &self.focus, direction)
            .map(|win_id| self.set_focus(win_id))
            .ok();
    }

    fn shift_focus(&mut self, order: frame::WindowOrder) {
        self.frame
            .get_sequent_window(&self.frame_ctx, &self.focus, order, true)
            .map(|win_id| self.set_focus(win_id))
            .ok();
    }
    ...
    fn set_focus(&mut self, win_id: frame::WindowId) {
        ...
        self.windows
            .get(&win_id)
            .map(|win| self.cmd_thread.set_mode(win.normal_mode.clone(), 1));
        self.windows
            .get_mut(&self.focus)
            .map(|win| win.needs_redraw = true);
    ...
    fn split_window(&mut self, orientation: frame::Orientation) {
        self.frame
            .split_window(&mut self.frame_ctx, &self.focus, orientation)
            .map(|new_win_id| {
                let win = self
                    .windows
                    .get(&self.focus)
    ...
    fn resize_window(&mut self, orientation: frame::Orientation, amount: isize) {
        self.frame
            .resize_window(&self.frame_ctx, &self.focus, orientation, amount)
            .map(|absorbed| {
                if absorbed != 0 {
                    self.invalidate_frame();
                }
    ...
    fn close_window(&mut self) {
        self.frame
            .get_closest_neighbouring_window(&self.frame_ctx, &self.focus)
            .map(|neighbour| {
                let old_focus = self.focus.clone();
                self.set_focus(neighbour);
                self.frame
                    .close_window(&mut self.frame_ctx, &old_focus)
                    .unwrap();
    ...
    fn draw_window(&self, win_id: &frame::WindowId, screen: &mut Screen) {
        self.windows
            ...
            .map(|win| {
                ...
                self.buffers.get(&win.buf_id).map(|buffer| {
                    win.view()
                        .draw(buffer, *win.caret(), focused, position, screen)
                })
            })
    ...
    fn invalidate_frame(&mut self) {
        let window_rects: Vec<(frame::WindowId, screen::Rect)> = self
            .windows
            .iter()
            .map(|(win_id, ref win)| {
                ...
                (
                    ...
                    old_rect,
                    self.frame
                        .get_window_rect(&self.frame_ctx, win_id)
                        .ok()
                        .expect("Couldn't find rect for window."),
                )
            })
        ...
        match cmd {
            ...
            WinCmd::EnterNormalMode => {
                ...
                self.buffers.remove(&id).map(|buffer| {
                    ...
                    for (_, win) in self.windows.iter_mut() {
                        win.caret_mut_for(id)
                            .map(|caret| caret.adjust(caret::Adjustment::Clamp, &buffer));
                    }
            ...
            WinCmd::EnterInsertModeAppendEndOfLine => {
                let col = self
                    .buffers
                    .get(&win.buf_id)
                    .map(|buf| buf.line_length(win.caret().line()).unwrap())
                    .unwrap();
            ...
            WinCmd::SaveBuffer => {
                self.buffers
                    .get(&win.buf_id)
                    .map(|buffer| buffer.write().ok().expect("Failed to save buffer."));
            }
            ...
            WinCmd::Backspace => {
                ...
                self.buffers
                    .get(&win.buf_id)
                    .map(|buffer| start.adjust(caret::Adjustment::CharPrevFlat, buffer));
            ...
            WinCmd::Delete => {
                ...
                self.buffers
                    .get(&win.buf_id)
                    .map(|buffer| end.adjust(caret::Adjustment::CharNextFlat, buffer));
            ...
            WinCmd::BackspaceOnLine => {
                ...
                self.buffers
                    .get(&win.buf_id)
                    .map(|buffer| start.adjust(caret::Adjustment::CharPrev, buffer));
                self.delete_range(start, win.caret().clone(), win);
            }
            WinCmd::DeleteOnLine => {
                let mut end = win.caret().clone();
                self.buffers
                    .get(&win.buf_id)
                    .map(|buffer| end.adjust(caret::Adjustment::CharNextAppending, buffer));
                self.delete_range(win.caret().clone(), end, win);
    ...
    fn insert(&mut self, string: String, win: &mut Window) {
        self.buffers.remove(&win.buf_id).map(|mut buffer| {
            let (insert_line, insert_col) = (win.caret().line(), win.caret().column());
            ...
            for c in string.chars() {
                ...
                let (new_line, new_col) = if newline {
                    (win.caret().line() + 1, 0)
                } else {
                    (win.caret().line(), win.caret().column() + 1)
                };
                win.caret_mut()
                    .adjust(caret::Adjustment::Set(new_line, new_col), &buffer);
                // update other windows which has viewed the buffer
                let id = win.buf_id;
                for (_, win) in self.windows.iter_mut() {
                    if !win.has_buf_id(id) {
                        continue;
                    }
                    // keep the content of other views still if possible
                    let (scroll_line, scroll_col) = win
                        .view_for(id)
                        .map(|v| (v.scroll_line(), v.scroll_column()))
                        .unwrap();
                    if scroll_line > c_line && newline {
                        win.view_mut_for(id)
                            .unwrap()
                            .set_scroll(scroll_line + 1, scroll_col);
                    }
                    // update caret of other window
                    let (cur_line, cur_col) =
                        win.caret_for(id).map(|c| (c.line(), c.column())).unwrap();
                    let (new_line, new_col) = if c_line < cur_line && newline {
                        (cur_line + 1, cur_col)
                    } else if cur_line == c_line && c_col <= cur_col {
                        if !newline {
                            (cur_line, cur_col + 1)
                        } else {
                            (cur_line, if c_col == 0 { 0 } else { c_col - 1 })
                        }
                    } else {
                        (cur_line, cur_col)
                    };
                    win.caret_mut_for(id)
                        .unwrap()
                        .adjust(caret::Adjustment::WeakSet(new_line, new_col), &buffer);
                    win.needs_redraw = true;
                }
                if newline {
                    c_line += 1;
            ...
            buffer
                .insert_at_line_column(string, insert_line, insert_col)
                .ok()
                ...
                .is_ok()
            {
                ...
                for (_, win) in self.windows.iter_mut() {
                    ...
                    let (scroll_line, scroll_col) = win
                        ...
                        .map(|v| (v.scroll_line(), v.scroll_column()))
                        .unwrap();
                    let new_scroll_line = if scroll_line <= start_line {
                        scroll_line
                    } else if scroll_line <= end_line {
                        start_line
                    } else {
                        scroll_line - end_line + start_line
                    };
                    win.view_mut_for(id)
                        .unwrap()
                        .set_scroll(new_scroll_line, scroll_col);
                    // update caret of other window
                    let (cur_line, cur_col) =
                        win.caret_for(id).map(|c| (c.line(), c.column())).unwrap();
                    let clamped_start_col = if start_col > 0 { start_col - 1 } else { 0 };
                    let new_col = if cur_line < start_line || cur_line > end_line {
                        cur_col
                    } else if start_line == end_line {
                        if cur_col < start_col {
                            cur_col
                        } else if cur_col < end_col {
                            clamped_start_col
                        } else {
                    ...
                    let new_line = if cur_line >= start_line && cur_line <= end_line {
                        start_line
                    } else if cur_line > end_line {
                        cur_line - end_line + start_line
                    } else {
                        cur_line
                    };
                    win.caret_mut_for(id)
                ...
                win.caret_mut()
                    .adjust(caret::Adjustment::Set(start_line, start_col), &buffer);
                let caret = *win.caret();
                win.view_mut().scroll_into_view(caret, &buffer);
                win.needs_redraw = true;
            }
...
fn main() {
    ...
    cmd_tx
        .unbounded_send(Cmd::WinCmd(WinCmd::OpenBuffer(PathBuf::from(&filename))))
        .unwrap();
                ...
                .map(|win| {
                    rim.buffers.get(&win.buf_id).map(|buffer| {
                        ...
                        screen.set_cursor_position(
                            win_position + win.view().caret_position(*win.caret(), buffer),
                        );
                    })
                })
                .expect("Couldn't find focused window.");
...
fn replace_mode(replace_line: bool) -> command::Mode {
    ...
    fn replace_line_fallback(key: Key) -> Option<Cmd> {
        key_to_string(key)
            .map(|string| Cmd::WinCmd(WinCmd::ReplaceLine(string)))
            .or(Some(Cmd::WinCmd(WinCmd::EnterNormalMode)))
    }
    fn replace_fallback(key: Key) -> Option<Cmd> {
        key_to_string(key)
            .map(|string| Cmd::WinCmd(WinCmd::Replace(string)))
            .or(Some(Cmd::WinCmd(WinCmd::EnterNormalMode)))
    }
    mode.fallback = if replace_line {
        replace_line_fallback
    } else {
        replace_fallback
    };
    return mode;
