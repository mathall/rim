...
impl View {
    ...
    pub fn caret_position(&self, caret: Caret, buffer: &Buffer) -> screen::Cell {
        ...
        let caret_column = caret::buffer_to_screen_column(caret.line(), caret.column(), buffer)
            - self.scroll_column;
    ...
    pub fn scroll_into_view(&mut self, caret: Caret, buffer: &Buffer) {
        ...
        self.scroll_column = if start < self.scroll_column {
            start
        } else if end >= self.scroll_column + cols {
            end - cols + 1
        } else {
            self.scroll_column
        };
...
mod test {
    ...
    fn scroll_into_view_double_width() {
        ...
        let buffer =
            Buffer::open(&Path::new("tests/view/scroll_into_view_double_width.txt")).unwrap();
        ...
        view.scroll_into_view(caret, &buffer);
        assert_eq!(view.scroll_line(), 0);
        assert_eq!(view.scroll_column(), 9);
        caret.adjust(caret::Adjustment::Set(0, 3), &buffer);
        view.scroll_into_view(caret, &buffer);
        assert_eq!(view.scroll_line(), 0);
        assert_eq!(view.scroll_column(), 6);
    ...
    fn caret_position() {
        let mut caret = Caret::new();
        let buffer = Buffer::open(&Path::new("tests/view/caret_position.txt")).unwrap();
        let mut view = View::new();
        view.set_scroll(1, 1);
