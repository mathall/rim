...
impl PageTree {
    ...
    fn line_column_to_offset(&self, line: usize, column: usize) -> Option<usize> {
        self.line_start_and_end_offset(line)
            .and_then(|(line_start_offset, line_end_offset)| {
    ...
    fn line_start_and_end_offset(&self, line: usize) -> Option<(usize, usize)> {
        self.offset_of_line_start(line).map(|line_start_offset| {
            let line_end_offset = self.offset_of_line_start(line + 1).unwrap_or(self.length);
    ...
    fn offset_of_line_start(&self, line: usize) -> Option<usize> {
        ...
        branch
            .as_ref()
            .and_then(|boxed_node| match **boxed_node {
                Tree(ref tree) => tree.offset_of_line_start(new_line),
                Leaf(ref page) => {
                    if new_line == 0 {
                        Some(0)
                    } else {
                        page.offset_of_newline(new_line - 1).map(|ofs| ofs + 1)
                    }
                }
            })
    ...
    fn get_char_by_line_column(&self, line: usize, column: usize) -> Option<char> {
        self.line_column_to_offset(line, column)
            .and_then(|offset| self.get_char_by_offset(offset))
    }
    ...
    fn get_char_by_offset(&self, offset: usize) -> Option<char> {
        ...
        branch.as_ref().and_then(|boxed_node| match **boxed_node {
    ...
    fn insert_string_at_offset(&mut self, string: String, offset: usize) {
        ...
        assert!(offset <= self.length);
        {
            ...
            match *branch {
                None => *branch = Some(Box::new(Leaf(Page::new(string)))),
                Some(ref mut boxed_node) => match **boxed_node {
                    Tree(ref mut tree) => tree.insert_string_at_offset(string, new_offset),
                    Leaf(ref mut page) => page.insert_string_at_offset(string, new_offset),
                },
            }
    ...
    fn insert_page_at_offset(&mut self, page: Page, offset: usize) {
        ...
        // insert the page in the appropriate branch
        {
            ...
            let insert_mode = branch
                .as_ref()
                .map(|boxed_node| match **boxed_node {
        ...
        let deleted = {
            ...
            if let Some(mut boxed_node) = branch.take() {
                ...
                *branch = if !page_emptied {
                ...
                panic!("Attempted deleting from None node");
            }
        };
    ...
    fn ensure_balanced(&mut self) {
        ...
        if height_diff == 2 && (left_height == 0 || right_height == 0) {
                ...
                &mut Some(ref mut boxed_node) => {
                    match **boxed_node {
                        ...
                        Tree(ref mut mid) => {
                            ...
                            let (branch, other) = if then_go_left {
                                (&mut mid.right, &mut mid.left)
                            } else {
                                (&mut mid.left, &mut mid.right)
                            };
            ...
            if first_go_left {
                mem::swap(&mut self.right, &mut t3);
            } else {
                mem::swap(&mut self.left, &mut t0);
            }
    ...
    fn find_page_by_offset(&self, offset: usize) -> Option<(&Page, usize)> {
        ...
        branch
            ...
            .map(|(page, offset)| (page, offset + if go_left { 0 } else { self.left.length() }))
    }
...
impl<'l> Iterator for PageTreeIterator<'l> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.tree
            .find_page_by_offset(self.next_offset)
            .map(|(page, offset)| {
                self.next_offset = offset + page.length;
                page
            })
    }
}
...
impl<'l> CharIterator<'l> {
    fn new(tree: &'l PageTree, start: usize, end: usize) -> CharIterator<'l> {
        ...
        for _ in 0..((start + page.length) - pages.next_offset) {
            chars.next();
        }
        CharIterator {
            counter: end - start,
...
impl<'l> Iterator for CharIterator<'l> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.counter == 0 {
            ...
            self.chars
                .as_mut()
                .and_then(|ref mut chars| chars.next())
                .or_else(|| {
...
impl<'l> Iterator for LineIterator<'l> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.tree
            .line_start_and_end_offset(self.next_line)
            .and_then(|(start, end)| {
                if start != end {
                    Some((start, end))
                } else {
...
impl Page {
    ...
    fn split(mut self) -> (Page, Page) {
        ...
        self.data = chunker
            .next()
            .expect("Split should have got a first chunk.");
        let rest = chunker
            .next()
        ...
        while self.data.len() > 0 || chunk.len() > 0 {
            chunk = match String::from_utf8(chunk) {
                ...
                Err(err) => {
                    ...
                    broken_chunk
                        .pop()
                        .map(|byte| self.data.insert(0, byte))
                        .unwrap();
                    broken_chunk
                }
            }
...
impl Buffer {
    ...
    fn ensure_ends_with_newline(&mut self) {
        let ends_with_newline = self.tree.length > 0
            && self
                .tree
                .get_char_by_offset(self.tree.length - 1)
                .map(|c| c == '\n')
    ...
    pub fn insert_at_line_column(
        ...
        self.tree
            .line_column_to_offset(line, column)
    ...
    pub fn delete_range(
        ...
        self.tree
            .line_column_to_offset(start_line, start_column)
            .and_then(|start| {
                self.tree
                    .line_column_to_offset(end_line, end_column)
                    .and_then(|end| {
            ...
            .map(|(start, mut end)| {
                while start < end {
                    end -= self.tree.delete_range(start, end);
                }
            })
            .ok_or(Error::BadLocation)
    }

    pub fn get_char_by_line_column(&self, line: usize, column: usize) -> Option<char> {
    ...
    pub fn line_length(&self, line: usize) -> Option<usize> {
        self.tree
            .line_start_and_end_offset(line)
            .and_then(|(start, end)| {
                if start >= end {
                    None
                } else {
...
mod test {
    ...
    fn is_balanced(tree: &PageTree) -> bool {
        let branch_is_balanced = |branch: &PageTreeLink| match *branch {
            Some(ref boxed_node) => match **boxed_node {
    ...
    buffer_test!(
        page_split_utf8_insert,
        false,
        page_split_utf8_insert_operation
    );
    ...
    fn long_string_insert_operation(buffer: &mut Buffer) {
        buffer.insert_at_offset(
            include_str!("../tests/buffer/long_string_insert.txt").to_string(),
            1,
        );
    }
    ...
    fn line_column_to_offset_test() {
        ...
        for &((line, column), expected_offset) in tests.iter() {
            assert_eq!(
                buffer.tree.line_column_to_offset(line, column),
                expected_offset
            );
        }
    }
    ...
    buffer_test!(
        delete_char_by_char_start,
        false,
        delete_char_by_char_start_op
    );
    ...
    fn delete_char_by_char_start_op(buffer: &mut Buffer) {
        for _ in 0..100 {
            buffer.tree.delete_range(0, 1);
        }
    }

    fn delete_char_by_char_mid_op(buffer: &mut Buffer) {
        for _ in 0..100 {
            buffer.tree.delete_range(68, 69);
        }
    }

    fn delete_char_by_char_end_op(buffer: &mut Buffer) {
        for i in 0..100 {
            buffer.tree.delete_range(255 - i, 256 - i);
        }
    }
