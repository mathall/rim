...
impl Section {
    ...
    where
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next()
            .map(|&go| match self.split {
                None => panic!("split: Bad path"),
                Some(ref mut split) => {
                    let (fst_path, snd_path) = (if go == Fst {
                        &mut split.fst
                    } else {
                        &mut split.snd
                    })
            ...
            .unwrap_or_else(|| {
                ...
                split.fst.size = if orientation == Vertical {
                    screen::Size(rows, fst_size)
                } else {
                    screen::Size(fst_size, cols)
                };
                split.snd.size = if orientation == Vertical {
                    screen::Size(rows, snd_size)
                } else {
                    screen::Size(snd_size, cols)
                };
                self.split = Some(split);
                (
                    SectionPath::new().prepend(Fst),
                    SectionPath::new().prepend(Snd),
                )
            })
    }
    ...
    fn reset_sizes_along(&mut self, orientation: Orientation) {
        ...
        self.split.as_mut().map(|ref mut split| {
            ...
            if split.orientation == orientation {
                ...
                let snd_size = snd_splits * BORDER_SIZE + distributable_size - fst_portion;
                assert!(fst_size >= fst_sections * MIN_SECTION_SIZE + fst_splits * BORDER_SIZE);
                assert!(snd_size >= snd_sections * MIN_SECTION_SIZE + snd_splits * BORDER_SIZE);
        ...
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next()
            .map(|&go| match self.split {
                ...
                Some(ref split) => (if go == Fst { &split.fst } else { &split.snd })
    ...
    fn num_splits_along(&self, orientation: Orientation) -> u16 {
        self.split.as_ref().map_or(0, |ref split| {
            ...
            let snd_splits = split.snd.num_splits_along(orientation);
            if split.orientation == orientation {
                cmp::max(fst_splits, snd_splits)
            } else {
                fst_splits + snd_splits + 1
            }
        })
    }
        ...
        path.next()
            ...
            .unwrap_or_else(|| {
                ...
                self.split = self.split.take().and_then(|mut split| {
                    let (either, other) = if side == Fst {
                        (&mut split.fst, &mut split.snd)
                    } else {
                        (&mut split.snd, &mut split.fst)
                    };
                    let screen::Size(rows, cols) = either.size;
                    let size = if split.orientation == Vertical {
                        cols
                    } else {
                        rows
                    };
                    let shift =
                        if side == Fst { -1 } else { 1 } * (BORDER_SIZE as isize + size as isize);
    ...
    fn get_aligning_base<'l, It>(&self, path: &mut It) -> Option<(SectionPath, Orientation, bool)>
    where
        It: Iterator<Item = &'l SectionSide>,
    {
    ...
    fn get_leaf_from<'l, It>(&self, path: &mut It, side: SectionSide) -> SectionPath
    where
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next()
    ...
    fn get_leaf(&self, side: SectionSide) -> SectionPath {
        match self.split {
            ...
            Some(ref split) => (if side == Fst { &split.fst } else { &split.snd })
                .get_leaf(side)
                .prepend(side),
        }
    }
    ...
    fn get_section_at_point(
        &self,
        position: screen::Cell,
        point: screen::Cell,
    ) -> Option<SectionPath> {
        match self.split {
            None => {
            ...
            Some(ref split) => {
                ...
                let border_rect = border_rect(position, split.orientation, split.fst.size);
    ...
    fn get_rect<'l, It>(&self, path: &mut It, position: screen::Cell) -> screen::Rect
    where
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next()
            .map(|&go| match self.split {
                ...
                Some(ref split) => {
                    let next_position = if go == Fst {
                        position
                    } else {
                        snd_position(position, split.fst.size, split.orientation)
                    };
                    (if go == Fst { &split.fst } else { &split.snd }).get_rect(path, next_position)
                }
            })
    ...
    where
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next().and_then(|&go| match self.split {
            None => panic!("get_adjacent: Bad path"),
            Some(ref split) => (if go == Fst { &split.fst } else { &split.snd })
                .get_adjacent(path, orientation, side)
                .map(|path| path.prepend(go))
                .or(if split.orientation == orientation || go == side {
                    None
                } else {
                    Some(SectionPath::new().prepend(side))
                }),
        })
    }
        ...
        It: Iterator<Item = &'l SectionSide>,
    {
        path.next()
            ...
            .unwrap_or_else(|| match self.split {
                None => 0,
                Some(ref mut split) => {
                    ...
                    let (either, other, either_side, other_side) = if amount < 0 {
                        (&mut split.fst, &mut split.snd, Snd, Fst)
                    } else {
                        (&mut split.snd, &mut split.fst, Fst, Snd)
                    };

                    let shift_orientation = split.orientation.opposite();
                    let either_took = either.shift_edge(shift_orientation, either_side, amount);
                    let other_took = other.shift_edge(shift_orientation, other_side, either_took);
    ...
    // A negative amount implies shifting in the top/left direction.
    // Returns the absorbed amount.
    fn shift_edge(&mut self, orientation: Orientation, side: SectionSide, amount: isize) -> isize {
        let screen::Size(rows, cols) = self.size;
        ...
        let took = match self.split {
            ...
            Some(ref mut split) => {
                if split.orientation == orientation {
                    ...
                    let fst_splits = split.fst.num_splits_along(orientation);
                    let snd_splits = split.snd.num_splits_along(orientation);
                    let (either, other) = if fst_splits > snd_splits {
                        (&mut split.fst, &mut split.snd)
                    } else {
                        (&mut split.snd, &mut split.fst)
                    };
    ...
    fn draw_borders(&self, position: screen::Cell, screen: &mut Screen) {
        self.split.as_ref().map(|ref split| {
            ...
            for screen_cell in
                screen::CellIterator::new(border_rect(position, split.orientation, split.fst.size))
            {
                let border_char = ' ';
...
fn border_rect(
    position: screen::Cell,
    orientation: Orientation,
    screen::Size(fst_rows, fst_cols): screen::Size,
) -> screen::Rect {
    let border_position = position
        + if orientation == Vertical {
            screen::Cell(0, fst_cols)
        } else {
            screen::Cell(fst_rows, 0)
        };
    let border_size = if orientation == Vertical {
        screen::Size(fst_rows, BORDER_SIZE)
    } else {
        screen::Size(BORDER_SIZE, fst_cols)
    };
    screen::Rect(border_position, border_size)
}

fn snd_position(
    fst_position: screen::Cell,
    fst_size: screen::Size,
    orientation: Orientation,
) -> screen::Cell {
...
impl<'l> LeafSectionIterator<'l> {
    ...
    fn find_next(&self, mut path: SectionPath) -> Option<SectionPath> {
        ...
        if path.len() == 0 && done_with_root {
            ...
            Some(self.main_section.get_leaf_from(
                &mut path.append(self.side).iter(),
...
impl error::Error for Error {
    fn description(&self) -> &str {
        match *self {
            ...
            Error::CantCloseLastWindow => "Attempted to close the last window in the frame.",
            Error::NoNeighbouringWindow => "The window has no neighbours. (it's the last window)",
...
impl Frame {
    pub fn new() -> (Frame, FrameContext, WindowId) {
        ...
        let mut ctx = FrameContext {
            window_to_section_path: HashMap::new(),
        };
    ...
    pub fn split_window(
        ...
        ctx.get_section_path(window)
            .map(|path| {
                ...
                let screen::Rect(_, screen::Size(rows, cols)) = self.get_section_rect(path);
                ...
                if size < min_size {
                    ...
                    let resized = self
                        .resize_window(ctx, window, grow_orientation, increase)
                        .unwrap();
                    if resized < increase {
                        // make room in the section, guaranteeing the window resize to succeed
                        let force_increase = increase - resized;
                        self.main_section
                            .shift_edge(grow_orientation, Snd, force_increase);
                        self.resize_window(ctx, window, grow_orientation, force_increase)
                            .unwrap();
                    }
                }
    ...
    pub fn close_window(&mut self, ctx: &mut FrameContext, window: &WindowId) -> Result<()> {
        ctx.get_section_path(window)
            .map(|path| path.clone())
            .and_then(|path| {
                ...
                parent_section_path
                    ...
                    .map(|side| {
                        ...
                        let reset_orientation = self
                            .main_section
                            .get_aligning_base(&mut path.iter())
                            .map(|(_, orientation, _)| orientation)
                            .unwrap()
                            .opposite();

                        // remove the section
                        self.main_section
                            .cut_branch(&mut parent_section_path.iter(), side);
                        ...
                        let (section_size, frame_size) = if reset_orientation == Vertical {
                            (section_rows, frame_rows)
                        } else {
                            (section_cols, frame_cols)
                        };
                        if section_size > frame_size {
                            let size_diff = frame_size as isize - section_size as isize;
                            self.main_section
                                .shift_edge(reset_orientation, Snd, size_diff);
                        } else {
    ...
    pub fn get_closest_neighbouring_window(
        &self,
        ctx: &FrameContext,
        window: &WindowId,
    ) -> Result<WindowId> {
                        ...
                        self.get_sequent_window(ctx, window, NextWindow, false)
                            .and_then(|next_window| {
                                ctx.get_section_path(&next_window)
                                    ...
                                    .or_else(|_| {
                                        panic!("Couldn't get section path for existing window.")
                                    })
                                    .and_then(|next_path| {
                                        let next_is_neighbour =
                                            aligning_base_path.does_prefix(&next_path);
                                        if next_is_neighbour {
                                            Ok(ctx.get_window(&next_path))
                                        } else {
                                            Err(Error::NoNeighbouringWindow)
                                        }
                                    })
                            })
                            .or_else(|_| {
    ...
    pub fn get_sequent_window(
        ...
        ctx.get_section_path(window)
            .and_then(|path| {
                LeafSectionIterator::new(&self.main_section, side)
                    .from(path.clone())
                    .next()
                    .or_else(|| {
                        if wrap {
                            LeafSectionIterator::new(&self.main_section, side).next()
                        } else {
                            None
                        }
                    })
        ...
        self.get_window_rect(ctx, window)
            .and_then(|rect| {
                adjacent_window_point(rect, direction)
                    .and_then(|point| {
                        self.main_section
                            .get_section_at_point(screen::Cell(0, 0), point)
                    })
    ...
    pub fn get_window_rect(&self, ctx: &FrameContext, window: &WindowId) -> Result<screen::Rect> {
        ctx.get_section_path(window)
            .map(|path| self.get_section_rect(path))
    }

    fn get_section_rect(&self, path: &SectionPath) -> screen::Rect {
        self.main_section
            .get_rect(&mut path.iter(), screen::Cell(0, 0))
    }
    ...
    pub fn resize_window(
        ...
        ctx.get_section_path(window).map(|path| {
            let clamped_amount = if amount > 0 {
                amount
            } else {
                let screen::Rect(_, screen::Size(rows, cols)) = self.get_section_rect(path);
            ...
            let snd_took = self.resize_section_recursive(path, orientation, Snd, clamped_amount);
            let fst_took =
                self.resize_section_recursive(path, orientation, Fst, clamped_amount - snd_took);
            fst_took + snd_took
        })
    }
    ...
    fn window_min_size(
        ...
        ctx.get_section_path(window)
            ...
            .map(|(aligning_base_path, aligning_base_orientation, _)| {
                if aligning_base_orientation != orientation {
                    MIN_SECTION_SIZE
                } else {
                    let num_splits = self
                        .main_section
                        .num_splits_along_at(orientation, &mut aligning_base_path.iter());
    ...
    fn resize_section(
        ...
        self.main_section
            .get_adjacent(&mut path.iter(), orientation, side)
...
fn adjacent_window_point(rect: screen::Rect, direction: Direction) -> Option<screen::Cell> {
    let screen::Rect(screen::Cell(row, col), screen::Size(rows, cols)) = rect;
    let sub = BORDER_SIZE + 1;
    match direction {
        Direction::Left => {
            if col >= sub {
                Some(screen::Cell(row, col - sub))
            } else {
                None
            }
        }
...
mod test {
    ...
    fn check_size_invariant_along(frame: &Frame, orientation: Orientation) {
        ...
        let (frame_size, section_size) = if orientation == Vertical {
            (frame_rows, section_rows)
        } else {
            (frame_cols, section_cols)
        };
    ...
    fn equal_subsection_size_along(section: &Section, orientation: Orientation) -> bool {
        match section.split {
            None => true,
            Some(ref split) => {
                ...
                let (size, fst_size, snd_size) = if orientation == Vertical {
                    (rows, fst_rows, snd_rows)
                } else {
                    (cols, fst_cols, snd_cols)
                };
    ...
    fn setup_frame(descriptor_nr: usize) -> (Frame, FrameContext, Vec<WindowId>) {
        ...
        for &(window_nr, orientation) in SPLIT_DESCRIPTORS[descriptor_nr].iter() {
            let new_window = frame
                .split_window(&mut ctx, &windows[window_nr], orientation)
                .unwrap();
    ...
    fn split_beyond_frame_size_and_close() {
        ...
        for window_nr in 0..100 {
            let new_window = frame
                .split_window(&mut ctx, &windows[window_nr], Vertical)
                .unwrap();
            windows.push(new_window);
            check_frame_invariants(&frame, &ctx);
        }
    ...
    fn set_size_frame_2() {
        ...
        let win_sizes = [
            ...
            screen::Size(win_rows, start_size - 70 - MIN_SECTION_SIZE - BORDER_SIZE),
            screen::Size(
                win_rows + win_2_rows + 10 - 50 - MIN_SECTION_SIZE,
                MIN_SECTION_SIZE,
            ),
            screen::Size(
                win_rows + win_2_rows + 10 - 50 - MIN_SECTION_SIZE,
    ...
    fn do_frame_resizes(
        ...
        for (&(add_rows, add_cols), win_size_expect) in adds.iter().zip(win_sizes.iter()) {
            let screen::Size(frame_rows, frame_cols) = frame.size;
            frame.set_size(screen::Size(
                (frame_rows as i16 + add_rows) as u16,
                (frame_cols as i16 + add_cols) as u16,
            ));
            check_frame_invariants(frame, ctx);
            let screen::Rect(_, win_size) = frame.get_window_rect(ctx, window).unwrap();
    ...
    fn do_window_resize(
        ...
        frame
            .resize_window(ctx, &windows[win], orientation, amount)
            .unwrap();
        check_frame_invariants(frame, ctx);
    ...
    fn try_closest_neighbours(frame_num: usize, closest_neighbours: &[(usize, usize)]) {
        let (frame, ctx, windows) = setup_frame(frame_num);
        for &(win, neighbour) in closest_neighbours.iter() {
            assert_eq!(
                windows[neighbour],
                frame
                    .get_closest_neighbouring_window(&ctx, &windows[win])
                    .unwrap()
            );
        }
    }
    ...
    fn try_window_sequence(frame_num: usize, sequence: &[usize]) {
        ...
        for &next_num in sequence.iter() {
            window = frame
                .get_sequent_window(&ctx, &window, order, true)
                .unwrap();
    ...
    fn get_adjacent_window_border_inclusive() {
        ...
        frame
            .resize_window(&ctx, &windows[0], Vertical, -1)
            .unwrap();
        check_frame_invariants(&frame, &ctx);
        frame
            .resize_window(&ctx, &windows[0], Horizontal, 1)
            .unwrap();
        check_frame_invariants(&frame, &ctx);
